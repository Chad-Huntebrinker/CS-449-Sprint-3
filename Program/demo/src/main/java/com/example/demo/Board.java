...
public class Board {
    ...
    public boolean checkForSOS(PlayerBox player, Board SOSBoard, GUI gui) {
        ...
        //Each variable is assigned to a certain SOS check.
        temp1 = checkHorizontal(player, SOSBoard, gui);
        temp2 = checkVertical(player, SOSBoard, gui);
        temp3 = checkLeftToRightDiagonal(player, SOSBoard, gui);
        temp4 = checkRightToLeftDiagonal(player, SOSBoard, gui);

        //If one of these "if" or "else if" statements are true, then that means a new SOS was formed.
        //If the first three are not true, then just return the 4th variable.
        //If the 4th variable is true, then an SOS was formed.  If it is false, then no
        //SOS was formed for any of the checks.
        if(temp1) {
            return temp1;
        }
        else if (temp2) {
